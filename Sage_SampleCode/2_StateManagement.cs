...
to keep its state correct from the perspective of external actors.")]
        public static void Run()
        {
            ...
            exec.RequestEvent(delegate
            {
                _tank.OpenFillValve();
            }, when, 1.0);
            exec.RequestEvent(delegate
            {
                _tank.CloseFillValve();
            }, when + TimeSpan.FromMinutes(15.0));
            exec.RequestEvent(delegate
            {
                _tank.OpenFillValve(40);
            }, when + TimeSpan.FromMinutes(30.0));
            for (int i = 0; i < 8; i++)
            {
        ...
        class Tank
        {
            ...
            private DateTime _levelAt;
            private long _fullEventID = -1;
            #endregion
            ...
            public Tank(IExecutive exec, double capacity, double initialLevel)
            {
                ...
                _exec.ExecutiveStarted += executive =>
                {
                    CloseFillValve();
            ...
            public double Level
            {
                get
                {
                    if (Math.Abs(_fillRate) > 1E-6)
                    {
                        double elapsedMinutes = ((TimeSpan)(_exec.Now - _levelAt)).TotalMinutes;
                        _level += _fillRate * elapsedMinutes;
                    }
                    _levelAt = _exec.Now;
                    return _level;
                }
            ...
            private void SetFillRate(double fillRate)
            {
                ...
                Console.Write("{0} : Setting tank fill rate to {1}, Level is currently {2}. ", _exec.Now, fillRate, level);
                _fillRate = fillRate;
                if (fillRate > 0.0)
                {
                    double minutesTilFull = ((_capacity - _level) / _fillRate);
                    _fullEventID = _exec.RequestEvent(TankIsFull, _exec.Now + TimeSpan.FromMinutes(minutesTilFull));
                ...
                else
                {
                    if (_fullEventID != -1)
                    {
                        _exec.UnRequestEvent(_fullEventID); // Rescind full event.
                    }
            ...
            {
                Console.WriteLine("{0} : Tank full notification received.", _exec.Now);
                _fullEventID = -1;
                CloseFillValve();
            }
...
of the handlers.")]
        ...
        private static void StartProcessToken(IExecutive exec, object userData)
        {
            ...
            exec.RequestEvent(
                delegate
                {
                    Console.WriteLine("{0} : n = {1}.", exec.Now, n++); /* Add a widget. */
                },
