...
namespace Highpoint.Sage.SimCore
{
    ...
    public class SPBInitializer : IXmlPersistable
    {
        ...
        public SPBInitializer(string key, double val, bool forceCreation)
        {
            ...
            _initType = InitType.Double;
        }

        /// <summary>
        /// Returns the type of this initializer, whether _bool, _double, or _string.
        /// </summary>
        public InitType InitializationType => _initType;
        ...
        public void Initialize(SmartPropertyBag spb)
        {
            ...
            if (theSpb.Contains(keys[i]))
            {
                ...
                {
                    case InitType.Bool:
                        theSpb[keys[i]] = BoolValue;
                        break;
                    case InitType.Double:
                        theSpb[keys[i]] = DoubleValue;
                        break;
                    case InitType.String:
                        theSpb[keys[i]] = StringValue;
                        break;
                }
            }
            else
            { // If it does not have such an entry, and we've been told to force creation,
              // then we create a new entry and set the value.
                if (ForceCreation)
                {
                    ...
                    {
                        case InitType.Bool:
                            theSpb.AddBoolean(keys[i], BoolValue);
                            break;
                        case InitType.Double:
        ...
        public virtual void SerializeTo(XmlSerializationContext xmlsc)
        {
            ...
            xmlsc.StoreObject("InitType", _initType);
            switch (_initType)
            {
                case InitType.Bool:
                    {
                        xmlsc.StoreObject("Value", BoolValue);
                        break;
                    }
                case InitType.Double:
                    {
                        xmlsc.StoreObject("Value", DoubleValue);
                        break;
                    }
                case InitType.String:
                    {
                        xmlsc.StoreObject("Value", StringValue);
                        break;
                    }
                default:
                    {
        ...
        public virtual void DeserializeFrom(XmlSerializationContext xmlsc)
        {
            ...
            _initType = (InitType)xmlsc.LoadObject("InitType");
            switch (_initType)
            {
                case InitType.Bool:
                    {
                        BoolValue = (bool)xmlsc.LoadObject("Value");
                        break;
                    }
                case InitType.Double:
                    {
                        DoubleValue = (double)xmlsc.LoadObject("Value");
                        break;
                    }
                case InitType.String:
                    {
                        StringValue = (string)xmlsc.LoadObject("Value");
                        break;
                    }
                default:
                    {
