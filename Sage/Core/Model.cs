...
namespace Highpoint.Sage.SimCore
{
    ...
    public class Model : IModel, IModelWithResources
    {
        ...
        public Model(string name, Guid guid)
        {
            ...
            _modelObjects = new ModelObjectDictionary();
        }
        #endregion

        #region >>> Manage Model Objects <<<
        private readonly ModelObjectDictionary _modelObjects;
        /// <summary>
        ...
        {
            _modelObjects.Add(modelObject.Guid, modelObject);
        }
        ...
        public ulong RandomSeed
        {
            ...
            set
            {
                ...
                {
                    throw new ApplicationException(msgReseedingInitializedRandomServer);
                }
            }
        }
        #endregion
        ...
        public ExecController ExecutiveController
        {
            ...
            set
            {
                if (_execController == null)
                {
                    ...
                    _execController.SetExecutive(this.Exec);
                }
                else
                {
        ...
        {
            if (_diagnostics)
                _Debug.WriteLine("Created " + resource.Name + ", which is of type " + resource.GetType().Name);
            if (ResourceCreatedEvent != null)
                ResourceCreatedEvent(resource);
        }
        ...
        {
            if (WarningHappened != null)
                WarningHappened(theWarning);
            _warnings.Add(theWarning);
        }
        /// <summary>
        ...
        {
            foreach (IModelError error in errors)
            {
        ...
        public bool AddError(IModelError theError)
        {
            ...
            errors.Add(theError.Target, theError);
        ...
        public virtual void Start()
        {
            ...
            IsPaused = false;
            IsCompleted = false;
            _stateMachine.DoTransition(GetStartEnum());
        }
        ...
        {
            if (_diagnostics)
                _Debug.WriteLine("Model.Pause() requested.");
            if (IsRunning)
            {
                IsRunning = true;
                IsReady = false;
        ...
        public virtual void Abort()
        {
            ...
            Exec.Stop();
            _stateMachine.DoTransition(GetAbortEnum());
            if (!IsReady)
            {
                IsRunning = false;
                IsReady = false;
        ...
        public virtual void Reset()
        {
            ...
            _stateMachine.DoTransition(GetIdleEnum());
            Debug.Assert(_stateMachine.State.Equals(GetIdleEnum()));
        ...
        private static readonly string msgReseedingInitializedRandomServer = "Attempting to set the RandomSeed value on a model that has already initialized the RandomServer. The RandomServer is initialized the first time it is accessed, so make sure that if you want to set the seed, it is done before that time.";
