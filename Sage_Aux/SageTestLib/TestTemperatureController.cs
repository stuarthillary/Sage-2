...
namespace Highpoint.Sage.Thermodynamics
{
    ...
    public class TemperatureControllerTester101
    {
        ...
        private void testTargeting(TCTestJig tj)
        {
        ...
        internal class TCTestJig
        {
            ...
            private readonly TemperatureController _tempController;
            public TemperatureController TempCtrl
            {
                get
                {
            ...
            public TCTestJig(double tSrc, double tMix, double tAmb, double tSet, double rampRatePerMinute, double err, TemperatureControllerMode tcMode, bool tcEnabled)
            {
                ...
                Initialize(brs);
                _mixture = new Mixture(null, "Test Mixture");
                brs.MyReactionProcessor.Watch(_mixture);
                Container container = new Container(1000, _mixture); // Container full volume is 1000 liters.
                _tempController = new TemperatureController(container);

                _mixture.AddMaterial(brs.MyMaterialCatalog["Water"].CreateMass(250, tMix)); // Add 250 kg.
                _mixture.AddMaterial(brs.MyMaterialCatalog["Sodium Nitrite"].CreateMass(100, tMix)); // Add 100 kg NaNO2.

                _tempController.AmbientTemperature = tAmb; // degreeC

                // Error band functionality has been obsoleted.
                //m_tempController.ErrorBand = err;         // +/- err degreeC dead band.
                _err = err; // Used for acceptability of non-precise results.

                _tempController.SetAmbientThermalConductance(.30, .25); // .25 W/degreeC
                _tempController.SetAmbientThermalConductance(.60, .50); // .50 W/degreeC
                _tempController.SetAmbientThermalConductance(.90, .75); // .75 W/degreeC
                _tempController.SetThermalConductance(.30, .40); // .4 W/degreeC
                _tempController.SetThermalConductance(.60, .80); // .8 W/degreeC
                _tempController.SetThermalConductance(.90, .120); // 1.2 W/degreeC

                _tempController.TCEnabled = tcEnabled; // Temperature control system is on.
                _tempController.TCMode = tcMode; // Temperature control system maintains a constant deltaT, or a constant tSrc.
                _tempController.TCSetpoint = tSet;
                _tempController.TCSrcTemperature = tSrc; // Syltherm (e.g.) temperature.
                _tempController.TCSrcDelta = tSrc; // To be used if/when the system is in constant delta mode.
                _tempController.TCTemperatureRampRate = new TemperatureRampRate(5.0, TimeSpan.FromMinutes(1));
            }
            ...
            public bool MixTempWithinTolerance
            {
                get
                {
                    return (Math.Abs(_mixture.Temperature - _tempController.TCSetpoint) <= _err);
                }
            }
